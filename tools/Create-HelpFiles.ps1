...
(
    ...
    [Parameter(Mandatory = $true)]
    [System.IO.DirectoryInfo]$BinaryOutputDirectory,
...
foreach ($member in $members)
{
    ...
    for ($i = 0; $i -lt $examples.count; $i++)
    {
        ...
        $example['Code'] = $code -replace $newLine, '%%%' -replace "\s{$extraWhitespaceCount}", '' -replace '%%%', $newLine -replace '\s+$', ''
        ...
        switch ($pair.Key)
        {
            ...
            'Examples'
            {
                $pair.Value | ForEach-Object {
                    ...
                    {
                        $text += $_.Code.Split($newLine) | Format-Text -MaxLineWidth $MaxLineWidth -TabCount 2 -KeepFormatting
                        $text += ''
                    }
